...
// ── Data & Storage ──
const KEYS = { venues: "c8-venues", events: "c8-events", orders: "c8-orders" };
...
const SAMPLE_EVENTS = [
  { id: "evt-001", venueId: "crooked8", title: "Neon Rodeo Night", date: "2026-04-11", time: "7:00 PM", doors: "6:00 PM", description: "An electrifying night of professional rodeo action under the lights. Bull riding, barrel racing, roping, and more — plus live country music and food vendors all evening long.", image: "🤠", category: "Rodeo", tickets: [{ type: "General Admission", price: 25, available: 300 }, { type: "Reserved Bleachers", price: 40, available: 100 }, { type: "VIP Chute-Side", price: 85, available: 30 }] },
  { id: "evt-002", venueId: "crooked8", title: "Scotty Mac & The Nugs", date: "2026-04-18", time: "8:00 PM", doors: "7:00 PM", description: "Fan-favorite Scotty Mac & The Nugs return to the Crooked 8 stage for a high-energy night of rock, country, and everything in between. Full bar and food trucks on site.", image: "🎸", category: "Live Music", tickets: [{ type: "General Admission", price: 20, available: 250 }, { type: "VIP Pit", price: 50, available: 40 }] },
  { id: "evt-003", venueId: "crooked8", title: "Kuna Stampede Days", date: "2026-06-20", time: "6:00 PM", doors: "5:00 PM", description: "The annual Kuna Stampede — three days of professional rodeo, carnival rides, live entertainment, food vendors, and community celebration. A Treasure Valley tradition at the Crooked 8.", image: "🐎", category: "Rodeo", tickets: [{ type: "Single Day - Adult", price: 18, available: 500 }, { type: "Single Day - Child (12 & Under)", price: 10, available: 500 }, { type: "3-Day Pass - Adult", price: 45, available: 200 }, { type: "3-Day Family 4-Pack", price: 120, available: 75 }] },
  { id: "evt-004", venueId: "crooked8", title: "Country Throwdown", date: "2026-04-25", time: "8:00 PM", doors: "7:00 PM", description: "A night of country's biggest hits performed live. Grab your boots, your hat, and your crew — line dancing, two-stepping, and ice-cold drinks under the Idaho sky.", image: "🎵", category: "Live Music", tickets: [{ type: "General Admission", price: 15, available: 300 }, { type: "Couples Pass", price: 25, available: 100 }] },
  { id: "evt-005", venueId: "crooked8", title: "Family Fun Fair", date: "2026-05-09", time: "11:00 AM", doors: "10:30 AM", description: "A daytime celebration for the whole family — games, face painting, petting zoo, pony rides, food trucks, live acoustic music, and plenty of room to run around.", image: "🎪", category: "Family", tickets: [{ type: "Adult", price: 12, available: 400 }, { type: "Child (12 & Under)", price: 6, available: 400 }, { type: "Family 4-Pack", price: 30, available: 100 }] },
  { id: "evt-006", venueId: "crooked8", title: "Treasure Valley Rock Fest", date: "2026-05-16", time: "5:00 PM", doors: "4:00 PM", description: "Three bands, one epic night. The Treasure Valley's best rock acts take the Crooked 8 stage back-to-back. Food trucks, full bar, and good times guaranteed.", image: "🎸", category: "Live Music", tickets: [{ type: "General Admission", price: 25, available: 350 }, { type: "VIP Front Stage", price: 60, available: 50 }] },
  { id: "evt-007", venueId: "crooked8", title: "Breakfast with Santa", date: "2026-12-13", time: "9:00 AM", doors: "8:30 AM", description: "Bring the kids for pancakes, hot cocoa, holiday crafts, and a visit with Santa Claus. Photo ops included. A beloved Crooked 8 holiday tradition.", image: "🎅", category: "Family", tickets: [{ type: "Adult", price: 15, available: 200 }, { type: "Child", price: 10, available: 200 }] },
  { id: "evt-008", venueId: "crooked8", title: "Open Riding Night", date: "2026-04-16", time: "5:00 PM", doors: "4:30 PM", description: "Bring your horse to the Crooked 8 arena for open riding. Waiver required — sign online before you arrive. All skill levels welcome.", image: "🐴", category: "Other Events", tickets: [{ type: "Rider Entry", price: 20, available: 40 }, { type: "Spectator", price: 0, available: 100 }] },
  { id: "evt-009", venueId: "crooked8", title: "Private Event & Wedding Showcase", date: "2026-05-03", time: "11:00 AM", doors: "10:30 AM", description: "Tour the Crooked 8 and see why it's the Treasure Valley's most unique event venue. Meet local caterers, photographers, DJs, and planners.", image: "💒", category: "Other Events", tickets: [{ type: "Free Admission (RSVP)", price: 0, available: 200 }] },
];
...
const useStorage = () => {
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem(KEYS.venues);
      const e = localStorage.getItem(KEYS.events);
      const o = localStorage.getItem(KEYS.orders);
  ...
  const save = useCallback((k, d) => { try { localStorage.setItem(k, JSON.stringify(d)); } catch (e) { console.error(e); } }, []);
  const updateEvents = useCallback((d) => { setEvents(d); save(KEYS.events, d); }, [save]);
  const updateOrders = useCallback((d) => { setOrders(d); save(KEYS.orders, d); }, [save]);
...
export default function App() {
  ...
  const purchase = () => {
    ...
    const oid = genId();
    const items = sel.tickets.map((t, i) => ({ type: t.type, qty: cart[i] || 0, price: t.price })).filter(i => i.qty > 0);
    const order = { id: oid, eventId: sel.id, venueId: venue.id, buyer: { ...buyer }, items, total: cartTotal, date: new Date().toISOString(), checkedIn: false };
    updateOrders([...orders, order]);
    updateEvents(events.map(ev => ev.id !== sel.id ? ev : { ...ev, tickets: ev.tickets.map((t, i) => ({ ...t, available: t.available - (cart[i] || 0) })) }));
    setLastOrder(order); setView("ticket"); setBuyer({ name: "", email: "", phone: "" }); setCart({});
  };

  const checkin = (oid) => updateOrders(orders.map(o => o.id === oid ? { ...o, checkedIn: true } : o));
      ...
      <div className="app">
        ...
        {view === "detail" && sel && <div className="sec fade" style={{ maxWidth: 800 }}>
          ...
          <div className="tkt-sec"><h3 className="dsp">Select Tickets</h3>
            ...
            <button className="buy" disabled={cartN===0} onClick={() => { if(cartN>0) setView("checkout"); }}>{cartN===0 ? "Select Tickets" : `Checkout · ${fmtCurrency(cartTotal)}`}</button>
          </div>
        </div>}

        {view === "checkout" && sel && <div className="sec fade" style={{ maxWidth: 500 }}>
          ...
          <div className="tkt-sec" style={{ marginBottom: 20 }}><h3 className="dsp">Your Info</h3>
            ...
            <div className="fr"><div className="fg"><label className="fl">Email *</label><input className="fi" type="email" value={buyer.email} onChange={e => setBuyer({...buyer,email:e.target.value})} placeholder="jane@email.com" /></div><div className="fg"><label className="fl">Phone</label><input className="fi" type="tel" value={buyer.phone} onChange={e => setBuyer({...buyer,phone:e.target.value})} placeholder="(208) 555-1234" /></div></div>
          </div>
          <div className="tkt-sec" style={{ marginBottom: 20 }}><h3 className="dsp">Order Summary</h3>{sel.tickets.map((t,i) => cart[i]>0 && <div className="cart-ln" key={i} style={{padding:"6px 0"}}><span>{cart[i]}× {t.type}</span><span>{fmtCurrency(cart[i]*t.price)}</span></div>)}<div className="cart-tot"><span>Total</span><span>{fmtCurrency(cartTotal)}</span></div></div>
          <div className="tkt-sec" style={{ marginBottom: 20 }}><h3 className="dsp">Payment</h3><p style={{color:"var(--text3)",fontSize:12,marginBottom:14}}>💳 Payment placeholder — connects to Stripe or Square in production.</p><div className="fg"><label className="fl">Card Number</label><input className="fi" placeholder="4242 4242 4242 4242" disabled style={{opacity:.5}}/></div><div className="fr"><div className="fg"><label className="fl">Exp</label><input className="fi" placeholder="MM/YY" disabled style={{opacity:.5}}/></div><div className="fg"><label className="fl">CVC</label><input className="fi" placeholder="123" disabled style={{opacity:.5}}/></div></div></div>
          <button className="buy" disabled={!buyer.name||!buyer.email} onClick={purchase}>Complete Purchase · {fmtCurrency(cartTotal)}</button>
        </div>}
